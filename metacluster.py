...
def merge_rows(list_rows,
    ...
    ht[19:19] = ['' if pd.isnull(best_row[col]) else str(best_row[col]) for col in colnames_filters]
    ...
    pct[21:21] = ['' if pd.isnull(best_row[col]) else str(best_row[col]) for col in colnames_filters]
    ht = '\t'.join(ht)
    pct = '\t'.join(pct)
...
def metaclustering(df, window,
                   metacluster_id, chrom, strand, primer, target_re,
...
def main(inputdir, pcdir, outputdir, window, target_re, blen):
    ...
    for hfile, pcfile in tqdm(paired_files):
        ...
        if not is_meta_table:
            ...
            colnames_pct.insert(12, 'RE_LIST')
            ...
            colnames_filters = htable_cols[21:]
